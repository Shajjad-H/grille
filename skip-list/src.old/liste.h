...
public:
  ...
  Cellule *firstCell(bool lastFloor = false) const;
  //Precondition : aucune
  //               (*this initialisee et manipulee uniquement a travers les
  //                operations du module)
  ...
  Cellule *nextCell(const Cellule *c, const size_t etage=0) const;
  ...
  Elem cellEle(const Cellule *const c) const;
  //Precondition : c adresse valide d'une Cellule de la Liste *this
...
private:
  ...
  size_t get_nb_etage() const;
  bool tooFar(const Cellule *c,const size_t etage,const Elem &e) const;
  Cellule * goDown(const Cellule *c,size_t etage) const;
  //Donnees membres-----------------------------------------------------------
  Cellule *ad;
};
